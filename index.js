...
const DEFAULT_PATHS = {
    ...
    ROLE_DEFINITIONS_PATH: process.env.ITER8_ROLE_DEFINITIONS_PATH || ".iter8/teams/role-definitions",
};
// === 角色文件映射 ===
    ...
    setupToolHandlers() {
                    ...
                    },
                    {
                        ...
                        inputSchema: {
                            ...
                            properties: {
                                workflow_id: {
                                    type: "string",
                                    description: "工作流ID (如: product-documentation, epic-story-breakdown等)",
                                },
                                inputs: {
                                    type: "object",
                                    description: "工作流输入参数",
                        ...
                        inputSchema: {
                            ...
                            properties: {
                                mode: {
                                    ...
                                    description: "思维模式 (RESEARCH/INNOVATE/PLAN/EXECUTE/REVIEW)",
                                },
                                context: {
                                    type: "string",
                                    description: "当前上下文信息",
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `工具执行失败: ${error instanceof Error ? error.message : "未知错误"}`);
            }
        });
    }
    // 激活角色
    async activateRole(args) {
        ...
        if (roleId === "architect" && this.isTechStackRequest(user_request)) {
            return await this.handleArchitectTechStackRequest(role, user_request, context);
        }
        // 特殊处理：文档创建请求的交互式信息收集
        if (this.isDocumentCreationRequest(user_request)) {
            return await this.handleDocumentCreationRequest(role, user_request, context);
        }
        // 加载角色上下文
...
*如需查看详细系统信息，请使用 \`*get_project_context\` 命令*`,
    ...
    // 加载角色配置文件
    async loadRoleConfig(roleId) {
        try {
            const configPath = path.join(process.cwd(), this.roleDefinitionsPath, `${roleId}.yml`);
            const exists = await fs
                .access(configPath)
                .then(() => true)
                .catch(() => false);
    ...
    async loadRoleContext(role) {
        ...
        for (const contextType of role.auto_load_context) {
            try {
                // 尝试加载相关文档和配置
                const contextPath = path.join(process.cwd(), DEFAULT_PATHS.DOCS_PATH, contextType);
                const exists = await fs
                    .access(contextPath)
                    .then(() => true)
                    .catch(() => false);
    ...
    // 获取协作建议
    getCollaborationSuggestions(roleId) {
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            participants: workflow.workflow?.sequence?.map((step) => step.agent) ||
                                [],
                            estimated_duration: workflow.workflow?.estimated_duration || "未指定",
                            status: "running",
                            current_step: firstStep?.step || null,
                            next_action: firstStep
                                ? `激活${firstStep.agent}角色执行${firstStep.action}`
                                : "等待配置",
                            message: `工作流 ${workflow.workflow?.name || workflow_id} 已启动 (${timeInfo.chineseDateTime})`,
                            time_info: {
    ...
    async listAvailableWorkflows() {
        ...
        try {
            ...
            const iter8WorkflowsPath = path.join(process.cwd(), ".iter8", "workflows");
            const iter8Files = await fs.readdir(iter8WorkflowsPath);
            workflows.push(...iter8Files
    ...
    async generateTemplate(args) {
        ...
        try {
            ...
            // 2. 加载模板文件
            const templatePath = path.join(process.cwd(), this.templatesPath, `${template_type}-tmpl.md`);
            const templateContent = await fs.readFile(templatePath, "utf8");
            // 3. 处理模板变量
            const processedContent = this.processTemplateVariables(templateContent, variables);
            // 4. 生成文件名和路径
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `✅ ${template_type} 文档已生成并保存到: ${filePath}\n\n📅 创建时间: ${timeInfo.currentDate} ${timeInfo.currentDateTime}\n\n🔍 文档预览:\n${processedContent.substring(0, 500)}...\n\n✨ 文档特点:\n- 使用当前实际日期: ${timeInfo.currentDate}\n- 不包含AI角色信息\n- 基于您提供的真实项目信息`,
                    },
                ],
            };
        }
    ...
    processTemplateVariables(template, variables) {
        ...
        const safeVariables = {
            ...
            "{{项目代号}}": variables.projectCode || variables.projectName || "[待填写项目代号]",
            ...
            "{{目标用户}}": this.formatArray(variables.targetUsers) || "[待填写目标用户]",
            "{{业务目标}}": this.formatArray(variables.businessObjectives) || "[待填写业务目标]",
    ...
    removeAIRoleReferences(content) {
        // 定义所有AI角色名称
        const aiRoleNames = [
            "姜尚",
            "嫦娥",
...
*模板生成时间: ${timeInfo.chineseDateTime}*`;
    ...
    // 促进协作
    async facilitateCollaboration(args) {
    ...
    isTechStackRequest(request) {
        ...
        return techKeywords.some((keyword) => request.toLowerCase().includes(keyword.toLowerCase()));
    }
    // 检测是否为文档创建请求
    isDocumentCreationRequest(request) {
        const docKeywords = [
            "创建",
            "生成",
            "写",
    ...
    // 处理@鲁班的技术选型请求
    async handleArchitectTechStackRequest(role, request, context) {
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({
                        ...
                        consultation_prompt: this.generateTechStackConsultationPrompt(request, context),
                        next_steps: [
                            "收集项目技术需求",
                            "分析团队技能和约束",
                            "提供3-5个技术栈方案",
    ...
    // 处理文档创建请求的交互式信息收集
    async handleDocumentCreationRequest(role, request, context) {
        const timeInfo = this.getCurrentTimeInfo();
        return {
            content: [
                {
...
请从第一个问题开始回答，我会根据您的回答继续深入了解项目需求。`,
    ...
    // 生成技术选型咨询提示
    generateTechStackConsultationPrompt(request, context) {
        return `## 🏗️ 技术栈选型咨询
...
请详细回答这些问题，我将基于您的具体情况提供专业的技术选型建议。`;
    ...
    // 生成信息收集提示
    generateInfoCollectionPrompt(role, request, context) {
...
**重要**: 我创建的文档将完全基于您提供的真实项目信息，确保专业性和可执行性。`;
    ...
    // 技术栈咨询方法
    async techStackConsultation(args) {
        const { project_type, requirements, team_skills = [], budget_constraints = "", timeline = "", } = args;
        // 生成技术栈方案
        const techStackOptions = this.generateTechStackOptions(project_type, requirements, team_skills);
        // 生成决策矩阵
        const decisionMatrix = this.generateDecisionMatrix(techStackOptions, requirements, team_skills);
        // 生成风险评估
        const riskAssessment = this.generateRiskAssessment(techStackOptions, requirements);
        // 生成实施路线图
        const roadmap = this.generateImplementationRoadmap(techStackOptions[0], requirements);
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({
                        ...
                        recommendations: this.generateRecommendations(techStackOptions, decisionMatrix),
                        next_steps: [
                            "选择最适合的技术栈方案",
                            "进行技术验证和原型开发",
                            "制定详细的实施计划",
    ...
    // 生成技术栈选项
    generateTechStackOptions(projectType, requirements, teamSkills) {
        const database = this.getTechStackDatabase();
        const projectKey = projectType.toLowerCase();
        const baseOptions = database[projectKey] || database["web"];
        return baseOptions.slice(0, 5).map((option) => ({
            ...option,
            team_fit_score: this.calculateTeamFitScore(option, teamSkills),
            complexity_adjusted: this.adjustComplexityForTeam(option.complexity, teamSkills),
        }));
    }
    // 生成决策矩阵
    generateDecisionMatrix(options, requirements, teamSkills) {
        const criteria = [
            "development_speed",
            "scalability",
            "maintainability",
        ...
        options.forEach((option) => {
            ...
            // 计算总分
            matrix.options[option.name].total_score = criteria.reduce((sum, criterion) => sum + matrix.options[option.name][criterion], 0);
        });
        return matrix;
    }
    // 生成风险评估
    generateRiskAssessment(options, requirements) {
    ...
    // 生成实施路线图
    generateImplementationRoadmap(topOption, requirements) {
        return {
    ...
    // 生成推荐建议
    generateRecommendations(options, decisionMatrix) {
    ...
    // 辅助评分方法
    calculateTeamFitScore(option, teamSkills) {
        ...
        const matchingSkills = teamSkills.filter((skill) => requiredSkills.some((tech) => tech.toLowerCase().includes(skill.toLowerCase())));
        return matchingSkills.length / requiredSkills.length;
    }
    adjustComplexityForTeam(complexity, teamSkills) {
        const teamFitScore = this.calculateTeamFitScore({ technologies: { frontend: teamSkills } }, teamSkills);
        if (teamFitScore > 0.7) {
    ...
    scoreDevelopmentSpeed(option) {
        const speedMap = { Low: 3, Medium: 6, High: 9 };
        return speedMap[option.complexity] || 5;
    }
    scoreScalability(option) {
        const scaleMap = { Low: 3, Medium: 6, High: 9 };
        return scaleMap[option.scalability] || 5;
    }
    scoreMaintainability(option) {
        return (option.pros.filter((pro) => pro.includes("维护")).length * 2 + 5);
    }
    scoreTeamExpertise(option, teamSkills) {
        return Math.round(this.calculateTeamFitScore(option, teamSkills) * 9);
    }
    scoreCostEffectiveness(option) {
        const costMap = { High: 3, Medium: 6, Low: 9 };
        return costMap[option.cost] || 5;
    }
    generateMitigationStrategies(option) {
        const strategies = [];
    ...
    // 声明思维模式
    async declareThinkingMode(args) {
