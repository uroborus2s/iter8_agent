...
class Iter8CLI {
  ...
  createRoleListCommand() {
    return new Command("list")
      ...
      .option(
        "-l, --layer <layer>",
        "按层级过滤 (business_value, technical_design, implementation, process_coordination)"
      )
  ...
  async activateRole(role, options) {
    ...
    console.log(
      chalk.cyan(
        `   ${roleInfo.mythological_title} → ${roleInfo.professional_title}`
      )
    );
    ...
    for (const [layerId, layerName] of Object.entries(layers)) {
      ...
      if (layerRoles.length > 0) {
        ...
        layerRoles.forEach((role) => {
          console.log(
            `  ${role.icon} ${chalk.green(role.name)} - ${role.title}`
          );
  ...
  async runWorkflow(workflow, options) {
    ...
    console.log(
      chalk.cyan("   使用 iter8 workflow status <execution-id> 查看状态")
    );
  }
  ...
  async generateTemplate(template, options) {
    ...
    try {
      variables.git_branch = execSync("git branch --show-current", {
        encoding: "utf8",
      }).trim();
      variables.git_user = execSync("git config user.name", {
        encoding: "utf8",
      }).trim();
    } catch (error) {
      // Git信息获取失败时使用默认值
